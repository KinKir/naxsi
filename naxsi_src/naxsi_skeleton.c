...
ngx_http_dummy_init(ngx_conf_t *cf)
{
  ...
  *h = ngx_http_dummy_access_handler;
  ...
  /* initalise internal rules for libinjection sqli/xss (needs proper special scores) */
  ...
  libjct_sql->sc_tag->data = ngx_pcalloc(cf->pool, 17 /* LIBINJECTION_SQL */);
  libjct_xss->sc_tag->data = ngx_pcalloc(cf->pool, 17 /* LIBINJECTION_XSS */);
...
static ngx_int_t ngx_http_dummy_access_handler(ngx_http_request_t *r)
{
  ...
  if (!ctx) {
    ...
    ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
		  "XX-LOOKUP : %p len:%d not_found:%d", lookup, lookup->len, lookup->not_found);
